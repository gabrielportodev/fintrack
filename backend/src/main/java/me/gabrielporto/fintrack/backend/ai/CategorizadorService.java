...
public class CategorizadorService {
    ...
    public CategorizadorService(
            ChatClient.Builder builder, CategoryRepository categoryRepository, AuthenticatedUserProvider userProvider) {
    ...
    public SuggestCategoryResponse sugerir(SuggestCategoryRequest request) {
        ...
        String nomes = categorias.stream()
    ...
    SuggestCategoryResponse resolver(CategorySuggestion sugestao, List<Category> categorias) {
        ...
        return categorias.stream()
                .filter(c ->
                        c.getName().equalsIgnoreCase(sugestao.categoryName().trim()))
                .findFirst()
